...
rm -rf dist
rm -rf fixed_wheels
export NN_PLUGIN_LIBRARY_DIR=$PREFIX/lib
export NN_PLUGIN_HEADER_DIR=$PREFIX/openmmapi/include
...
function repair() {
  ...
  if [[ "$target_platform" == "linux-"* ]]; then
    ...
    auditwheel repair dist/*.whl \
      ...
      --exclude libOpenMMTorchCUDAReference.so \
    ...
    $BUILD_PREFIX/bin/python $(which delocate-wheel) \
      ...
      --exclude libOpenMMTorchCUDA.dylib \
      --exclude libOpenMMTorchCUDAOpenCL.dylib \
      --exclude libOpenMMTorchCUDAReference.dylib
  fi
}
